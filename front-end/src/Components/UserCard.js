...
function UserCard({ profile }) {
  ...
  return (
    <>
      ...
      <img className={classes.image} src={image.url ? image.url : defaultProfile} alt="buddypic" style={keepAspectRatio} />
      <Box component="fieldset" mb={3} borderColor="transparent" className={classes.ratingBox}>
        <Typography component="legend" variant="h6" className={classes.ratingBox}>
          Karma Stars
        </Typography>

        <Rating name="read-only" value={Number(karma) - 0.4} readOnly precision={0.1} style={{ color: "rgb(139, 58, 231)" }} />
      </Box>
      ...
      {(window.location.pathname === "/search-results" || window.location.pathname === `/users/14/feed`) && (
        <Button variant="contained" className={classes.flex45} color="secondary">
          Match
        </Button>
      )}

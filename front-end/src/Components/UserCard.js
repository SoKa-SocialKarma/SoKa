...
function UserCard ({ profile }) {
  ...
  return (
    <>
      ...
      {(window.location.pathname.match(/search-results/gi) ||
        window.location.pathname.match(/feed/gi)) &&
        !window.location.pathname.match(/matches/gi) && (
          <Button
            variant='contained'
            className={classes.flex45}
            color={buttonColor}
            onClick={() => setButtonColor('default')}
          >
            {buttonColor === 'secondary' ? 'Match' : 'Requested'}
          </Button>
        )}
